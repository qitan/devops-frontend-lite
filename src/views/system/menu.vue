    ...
    <div class="table-page-search-wrapper">
        ...
        <a-row :gutter="48">
            ...
            <a-col :md="8" :sm="24">
                ...
                <a-input-number v-model="queryParam.component" style="width: 100%"/>
                </a-form-item>
            </a-col>
            <a-col :md="8" :sm="24">
...
import CreateForm from './modules/MenuForm'
...
data () {
    ...
    loadData: parameter => {
        ...
        return this.$http.getMenuList(requestParameters)
        .then(res => {
...
methods: {
    ...
    form.validateFields((errors, values) => {
        ...
        if (values.id > 0) {
            // 修改 e.g.
            this.$http.patchMenu(values).then(res => {
            this.visible = false
            ...
            // 新增
            this.$http.createMenu(values).then(res => {
                this.visible = false
            this.confirmLoading = false
            // 重置表单数据
            form.resetFields()
    ...
    handleDel (record) {
      this.$http.deleteMenu(record).then(res => {
        if (res.code === 20000) {
