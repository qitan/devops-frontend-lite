...
import CreateForm from './modules/EnvironmentForm'
...
},
{
    title: '启用工单',
    dataIndex: 'ticket_on',
    scopedSlots: { customRender: 'ticket_on' }
},
{
    title: '描述',
    dataIndex: 'desc',
...
data () {
    ...
    loadData: parameter => {
        ...
        return this.$http.getEnvironmentList(requestParameters)
        .then(res => {
            return res.data
        })
    },
...
methods: {
    ...
    form.validateFields((errors, values) => {
        ...
        values = { ...values, ticket_on: values.ticket_on ? 1 : 0 }
        if (values.id > 0) {
            // 修改 e.g.
            this.$http.patchEnvironment(values).then(res => {
            this.visible = false
            ...
            // 新增
            this.$http.createEnvironment(values).then(res => {
                this.visible = false
            this.confirmLoading = false
            // 重置表单数据
            form.resetFields()
    ...
    handleDel (record) {
      this.$http.deleteEnvironment(record).then(res => {
        if (res.code === 20000) {
